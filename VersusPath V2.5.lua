...
function VersusPath:StopMovement(Success:boolean)
	...
	pcall(task.cancel, self._MovementFunction) -- stop current run function
	self._MovementFunction = nil
	self._Humanoid:MoveTo(self._Character:GetPivot().Position)
	-- stops movement
	if self.MoveTimeoutTask then
		pcall(task.cancel, self.MoveTimeoutTask) -- stop timeout task
		self.MoveTimeoutTask = nil
	end
	--pcalls is here to don't stop function in cause some error with task cancel 
